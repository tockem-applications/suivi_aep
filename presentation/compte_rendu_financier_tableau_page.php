...
function generateCompteRenduTableauMois($id_aep, $mois_debut, $mois_fin) {
    ...
    foreach ($compte_rendu as $mois_data) {
        foreach ($mois_data['recettes'] as $libelle => $montant) {
            if (!isset($categories_recettes[$libelle]) && 
...
$mois_noms = array('', 'Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin', 
...
<div class="container-fluid mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="text-primary fw-bold">
            ...
            <?php else: ?>
                - Période <?php echo date('m/Y', strtotime($mois_debut . '-01')); ?> à <?php echo date('m/Y', strtotime($mois_fin . '-01')); ?>
            <?php endif; ?>
        </h2>
        <div class="d-flex flex-wrap align-items-center gap-2">
            <a href="?page=config_compte_rendu" class="btn btn-outline-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Configurer les libellés des catégories">
            ...
            <form method="get" action="" class="d-inline">
                ...
                <select name="type_periode" class="form-select d-inline-block" style="width: auto;" onchange="this.form.submit()">
                    <option value="annee" <?php echo $type_periode === 'annee' ? 'selected' : ''; ?>>Par Année</option>
                    <option value="mois" <?php echo $type_periode === 'mois' ? 'selected' : ''; ?>>Par Période (mois)</option>
                </select>
            </form>
            ...
            <?php else: ?>
                ...
                <form method="get" action="" class="d-inline">
                    ...
                    <div class="d-inline-flex align-items-center gap-2">
                        ...
                        <input type="month" id="mois_debut_select" name="mois_debut" class="form-control d-inline-block" style="width: auto;" value="<?php echo htmlspecialchars($mois_debut); ?>" onchange="this.form.submit()">
                        <label for="mois_fin_select" class="form-label mb-0">Mois fin:</label>
                        <input type="month" id="mois_fin_select" name="mois_fin" class="form-control d-inline-block" style="width: auto;" value="<?php echo htmlspecialchars($mois_fin); ?>" onchange="this.form.submit()">
                    </div>
                </form>
            <?php endif; ?>
            ...
            <div class="table-responsive" id="tableau_compte_rendu">
                <table class="table table-bordered table-sm mb-0" style="font-size: 0.85rem;">
                    <thead class="table-dark">
                        <tr>
                            <th style="min-width: 120px; max-width: 120px; width: 120px; position: sticky; left: 0; background: #212529; z-index: 10; box-shadow: 2px 0 5px rgba(0,0,0,0.2); padding: 0.5rem;">Code budgétaire</th>
                            <th style="min-width: 180px; max-width: 180px; width: 180px; position: sticky; left: 120px; background: #212529; z-index: 10; box-shadow: 2px 0 5px rgba(0,0,0,0.2); padding: 0.5rem;">Nom de la catégorie</th>
                            <?php if ($type_periode === 'annee'): ?>
                                <?php for ($m = 1; $m <= 12; $m++): ?>
                                    <th class="text-center" style="min-width: 80px; padding: 0.5rem;"><?php echo substr($mois_noms[$m], 0, 4); ?></th>
                                <?php endfor; ?>
                            <?php else: ?>
                                <?php foreach ($donnees['mois_list'] as $mois_key): ?>
                                    <th class="text-center" style="min-width: 80px; padding: 0.5rem;"><?php echo date('M Y', strtotime($mois_key . '-01')); ?></th>
                                <?php endforeach; ?>
                            <?php endif; ?>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr class="<?php echo $row_class; ?>">
                                <td style="position: sticky; left: 0; background-color: <?php echo $td_bg_color; ?> !important; z-index: 5; box-shadow: 2px 0 5px rgba(0,0,0,0.1); min-width: 120px; max-width: 120px; width: 120px; padding: 0.5rem;" class="fw-bold">
                                    <?php echo htmlspecialchars(isset($ligne['code_budgetaire']) && !empty($ligne['code_budgetaire']) ? $ligne['code_budgetaire'] : '-'); ?>
                                </td>
                                <td style="position: sticky; left: 120px; background-color: <?php echo $td_bg_color; ?> !important; z-index: 5; box-shadow: 2px 0 5px rgba(0,0,0,0.1); min-width: 180px; max-width: 180px; width: 180px; padding: 0.5rem;" class="fw-bold">
                                ...
                                ?>
                                <td class="text-end bg-primary text-white fw-bold <?php echo in_array($ligne['type'], array('resultat', 'tresorerie_branchements', 'tresorerie_vente_eau', 'tresorerie')) ? ($ligne['cumule'] < 0 ? 'text-danger' : 'text-success') : ''; ?>" style="padding: 0.5rem;">
    ...
    <div class="alert alert-info mt-3">
        ...
        <strong>Note :</strong> Les montants sont exprimés en FCFA. Les résultats négatifs indiquent une perte pour le mois concerné.
    </div>
</div>
