...
select a.id,
       a.nom                          as libele,
       numero_compteur                as numero,
       numero_telephone               as numero_abone,
       derniers_index                 as ancien_index,
       0.0                            as nouvel_index,
       r.nom                          as reseau,
       0.0                            as latitude,
       0.0                            as longitude,
...
from facture f,
     constante_reseau c,
     mois_facturation m
where m.id_constante = c.id
  and f.id_mois_facturation = m.id
...
select f.id,
       ...
       f.ancien_index,
       f.nouvel_index,
       c.prix_entretient_compteur,
       c.prix_metre_cube_eau,
       c.prix_tva,
       penalite,
       impaye,
       f.montant_verse,
...
from abone a,
     facture f,
     constante_reseau c
where f.id_mois_facturation = 45
  and c.id = 5
  and a.id = f.id_abone
order by a.id;
...
where a.id_reseau = r.id
  and f.id_abone = a.id
  and f.id_mois_facturation = m.id a.id = 1;
...
from abone a,
     ...
     mois_facturation m,
     reseau r
where f.id_mois_facturation = $id_mois
  and c.id = $id_constante
  and a.id = f.id_abone
order by a.id m.id=$id_mois;
...
from abone a,
     ...
     mois_facturation m,
     reseau r
where f.id_mois_facturation = m.id
  and c.id = m.id_constante
  and a.id = f.id_abone
  and m.id = 65
  and r.id = a.id_reseau
order by a.id;


select m.*                              mois,
       count(f.id)                      anombre_abone,
       sum(montant_verse)               total_verse,
       sum(nouvel_index - ancien_index) total_conso
from mois_facturation m,
     facture f
where m.id = 72
  and m.id = f.id_mois_facturation;

select *
from facture
where id_mois_facturation = 72
...
select a.id,
       ...
       m.mois,
       sum(nouvel_index - ancien_index) total_conso,
       sum(montant_verse)               montant_verse
from abone a,
     mois_facturation m,
     reseau r,
     facture f
where a.id = 6
  and a.id_reseau = r.id
  and a.id = f.id_abone
  and m.id = f.id_mois_facturation;
...
update abone a, facture f
set derniers_index = d_index
where f.id_abone = a.id and f.id_mois_facturation = max (id_mois_facturation)

select a.nom,
       count(f.id)                      duree,
       r.nom                            reseau,
       numero_telephone,
       sum(nouvel_index - ancien_index) conasommation,
       derniers_index,
       sum(impaye)                      impaye,
       numero_compteur,
       sum(montant_verse)               montant_verse,
       max(date_paiement)               date_paiement,
       a.etat
from abone a,
     facture f,
     reseau r
where a.id = f.id_abone
  and r.id = a.id_reseau
  and a.id = 7;
...
select m.*,
       f.id,
       count(f.id)                          nombre,
       sum(f.montant_verse)                 montant_versee,
       sum(f.nouvel_index - f.ancien_index) conso
from mois_facturation m,
     facture f,
     constante_reseau c
where m.id = f.id_mois_facturation
  and c.id = m.id_constante
group by m.id;
...
where mf.mois >= '2024-03'
  and mf.mois <= '2024-12'
ORDER BY mf.mois, a.id;
...
update mois_facturation m
    inner join constante_reseau c
on m.id_constante = c.id
